...
---------------------------------------------------------------------------------------------------
	...
	2021-09-28		김성호			INDEX 사용을 위해 서브쿼리 임시테이블 변환 작업
	2022-05-02		김성호			온라인팀 정산변경으로 출발일 조회 조건 변경 (SET_MASTER.DEP_DATE -> PKG_DETAIL.DEP_DATE)
-------------------------------------------------------------------------------------------------*/ 
CREATE PROCEDURE [dbo].[SP_SET_GET_RES_COMPLETE_LIST_5]
(
...
--		@TABLE_RES_LIST NVARCHAR(MAX) = '',
	...
	BEGIN
		...
		SET @WHERE1 = 'WHERE Z.DEP_DATE >= @START_DATE AND Z.DEP_DATE < @END_DATE'
		--SET @WHERE2 = 'WHERE A.DEP_DATE >= @START_DATE AND A.DEP_DATE < @END_DATE'
		SET @WHERE2 = 'WHERE B.DEP_DATE >= @START_DATE AND B.DEP_DATE < @END_DATE'
	END
	ELSE
	BEGIN
...
BEGIN
	...
	SET @SQLSTRING = N'
		-- 예약정보
		IF (OBJECT_ID(''tempdb..#RES_LIST'') IS NOT NULL)
		BEGIN
			DROP TABLE #RES_LIST;
		END
		CREATE TABLE #RES_LIST (PRO_CODE VARCHAR(20) NOT NULL, RES_CODE VARCHAR(20) NOT NULL, SIGN_CODE CHAR(1), SET_TYPE VARCHAR(10), SET_RATE DECIMAL);
		CREATE NONCLUSTERED INDEX #IDX_RES_LIST ON #RES_LIST (RES_CODE, PRO_CODE);

		-- 행사 1인 수익
		IF (OBJECT_ID(''tempdb..#PRO_LIST'') IS NOT NULL)
		BEGIN
			DROP TABLE #PRO_LIST;
		END
		CREATE TABLE #PRO_LIST (PRO_CODE VARCHAR(20) NOT NULL, ONE_PROFIT_PRICE DECIMAL);
		CREATE NONCLUSTERED INDEX #IDX_PRO_LIST ON #PRO_LIST (PRO_CODE);
		...
		END
		CREATE TABLE #LAST_LIST (PRO_CODE VARCHAR(20) NOT NULL, RES_CODE VARCHAR(20) NOT NULL, SIGN_CODE CHAR(1), SET_TYPE VARCHAR(10), SET_RATE DECIMAL
			, RES_COUNT INT, TOTAL_SALE_PRICE DECIMAL, ONE_PROFIT_PRICE DECIMAL);
		CREATE NONCLUSTERED INDEX #IDX_LAST_LIST ON #LAST_LIST (PRO_CODE, RES_CODE);
		
		-- 대상 예약 검색
		INSERT INTO #RES_LIST (PRO_CODE, RES_CODE, SIGN_CODE, SET_TYPE, SET_RATE)
		SELECT
			A.PRO_CODE, B.RES_CODE, C.SIGN_CODE,
		...
		INNER JOIN PKG_DETAIL Z WITH(NOLOCK) ON A.PRO_CODE = Z.PRO_CODE
		' + @WHERE1 + N'
		;'

	--	-- 대상 예약 검색
	--SET @TABLE_RES_LIST = N'
	--		INSERT INTO #RES_LIST (MASTER_CODE, PRO_CODE, RES_CODE, SIGN_CODE, SET_TYPE, SET_RATE)
	--		SELECT
	...
	SET @TABLE_RES_TOTAL_PRICE_LIST = N'
			SELECT MAX(C.PRO_CODE) AS [PRO_CODE], A.RES_CODE, COUNT(*) AS [RES_COUNT]
			...
			LEFT JOIN RES_CUSTOMER_damo B WITH(NOLOCK) ON A.RES_CODE = B.RES_CODE
			LEFT JOIN RES_MASTER_damo C WITH(NOLOCK) ON A.RES_CODE = C.RES_CODE
			WHERE A.RES_CODE IN (
				SELECT Z.RES_CODE 
				FROM RES_MASTER_damo Z WITH(NOLOCK) 
				WHERE Z.PRO_CODE IN (
					SELECT PRO_CODE FROM #RES_LIST Y GROUP BY Y.PRO_CODE
				) AND Z.RES_STATE <= 7
			) AND A.RES_CODE LIKE ''RH%'' AND B.RES_STATE IN (0, 3, 4)
			GROUP BY A.RES_CODE
			UNION ALL
			...
			WHERE A.RES_CODE IN (
				...
				WHERE Z.PRO_CODE IN (
					SELECT PRO_CODE FROM #RES_LIST Y GROUP BY Y.PRO_CODE
				) AND (Z.PRO_CODE LIKE ''KTR0%'' OR Z.RES_STATE <= 7)
			) AND A.RES_CODE LIKE ''R[P,T]%'' AND (B.PRO_CODE LIKE ''KTR0%'' OR A.RES_STATE IN (0, 3, 4))
			GROUP BY A.RES_CODE
			'
	...
	SET @TABLE_ONE_PROFIT_PRICE_LIST = N'
			...
			FROM (
				...
				FROM (
					...
					-- 항공기 거래처별 고객
						...
						WHERE PRO_CODE IN (SELECT DISTINCT PRO_CODE FROM #RES_LIST)
						GROUP BY PRO_CODE
					...
					LEFT OUTER JOIN (
						...
						FROM (
							...
							WHERE PRO_CODE IN (SELECT DISTINCT PRO_CODE FROM #RES_LIST) AND RES_STATE <= 7
							GROUP BY PRO_CODE
						) AS X
					...
					LEFT OUTER JOIN (
						...
						WHERE PRO_CODE IN (SELECT DISTINCT PRO_CODE FROM #RES_LIST) AND B.CXL_YN = ''N''
						GROUP BY B.PRO_CODE
			...
			WHERE A.TOTAL_RES_COUNT > 0
			'
	SET @SQLSTRING = @SQLSTRING + N'
		
		INSERT #LAST_LIST (PRO_CODE, RES_CODE, SIGN_CODE, SET_TYPE, SET_RATE, RES_COUNT, TOTAL_SALE_PRICE)
		SELECT A.PRO_CODE, A.RES_CODE, A.SIGN_CODE, A.SET_TYPE, A.SET_RATE, B.RES_COUNT, B.TOTAL_SALE_PRICE
		FROM #RES_LIST A
		INNER JOIN (' + @TABLE_RES_TOTAL_PRICE_LIST + N') B ON A.RES_CODE = B.RES_CODE AND B.RES_COUNT > 0;
		
		INSERT #PRO_LIST (PRO_CODE, ONE_PROFIT_PRICE)
		' + @TABLE_ONE_PROFIT_PRICE_LIST + N';
		
		UPDATE A SET A.ONE_PROFIT_PRICE = B.ONE_PROFIT_PRICE
		FROM #LAST_LIST A
		INNER JOIN #PRO_LIST B ON A.PRO_CODE = B.PRO_CODE;'
	...
	SET @TABLE_BRANCH_LIST = N'
			SELECT A.PRO_CODE, A.RES_CODE, A.RES_COUNT, A.SIGN_CODE, A.TOTAL_SALE_PRICE, A.SET_TYPE, A.SET_RATE, A.ONE_PROFIT_PRICE,
				(
					CASE A.SET_TYPE
	...
	SET @TABLE_GROUP_LIST = N'
			SELECT 1 AS BRANCH_ORDER, A.ONE_PROFIT_PRICE,
				...
				--A.RES_COUNT,
				A.TOTAL_SALE_PRICE,
				B.MASTER_CODE, A.PRO_CODE, A.RES_CODE, A.SIGN_CODE,
				(
					CASE A.SET_TYPE
